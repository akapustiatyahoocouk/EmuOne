...
gcc {
    ...
    EMUONE_BUILD_DATE=$$system(date '+%Y%m%d')
    EMUONE_BUILD_TIME=$$system(date '+%H%M')
    QMAKE_CXXFLAGS += -Wno-c++98-compat
}
msvc {
    QMAKE_CXXFLAGS += /Wall /D_CRT_SECURE_NO_WARNINGS
    EMUONE_BUILD_DATE=$$system(c:\\cygwin64\\bin\\date '+%Y%m%d')
    EMUONE_BUILD_TIME=$$system(c:\\cygwin64\\bin\\date '+%H%M')
    QMAKE_CXXFLAGS += /wd4996
}
TARGET_SUFFIX=-$$EMUONE_VERSION
DEFINES += EMUONE_VERSION=\\\"$$EMUONE_VERSION\\\"
DEFINES += EMUONE_BUILD_DATE=\\\"$$EMUONE_BUILD_DATE\\\"
DEFINES += EMUONE_BUILD_TIME=\\\"$$EMUONE_BUILD_TIME\\\"
INCLUDEPATH += ..
