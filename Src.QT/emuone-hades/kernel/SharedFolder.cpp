...
//////////
//  Operations
bool SharedFolder::isValid() const
{
...
{
    template <>
    QString toString<SharedFolder>(
            const SharedFolder & value
        )
    {
        return '<' +
               escape(value.volumeName()) +
               ',' +
               escape(value.hostPath()) +
               '>';
    }

    template <>
    QString toString<SharedFolders>(
            const SharedFolders & value
        )
    {
        QString result;
        result += '[';
    ...
    template <>
    SharedFolder fromString<SharedFolder>(
        ...
        )
    {
        ...
        //  Resolve mnemonic
        auto result = SharedFolder(chunks[0], chunks[1]);
        scan = prescan + 1;
        return result;
    }

    template <>
    SharedFolders fromString<SharedFolders>(
            const QString & s, qsizetype & scan
        )
    {
        //  Skip '['
        ...
        {   //  At least 1 item exists
            for (; ; )
            {
                SharedFolder workspaceAddress = fromString<SharedFolder>(s, prescan);   //  may throw
                if (!result.contains(workspaceAddress))
                {
                    result.insert(workspaceAddress);
                }
                //  More ?
                if (prescan < s.length() && s[prescan] == ',')
                {   //  yes
